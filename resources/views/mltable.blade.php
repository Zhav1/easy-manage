        ...
        <div class="p-6 border border-gray-300 rounded-xl shadow-sm bg-white">
            <div class="flex justify-between items-center mb-6">
                ...
                <button onclick="openAddItemModal()" class="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white px-6 py-2 rounded-full font-medium transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                 ...
                 <button onclick="exportToPDF()" class="bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white px-4 py-2 rounded-full font-medium transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
            <i class="fas fa-file-pdf mr-2"></i> Export PDF
        </button>
        <button onclick="exportToExcel()" class="bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-700 hover:to-emerald-800 text-white px-4 py-2 rounded-full font-medium transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
...
function exportToPDF() {
    ...
    doc.setProperties({
        title: 'Daftar Logistik RS',
        subject: 'Data Inventaris Logistik',
        author: 'Sistem Manajemen Logistik RS',
        keywords: 'logistik, inventaris, rumah sakit',
        creator: 'Sistem Manajemen Logistik RS'
    });
    
    // Add title
    doc.setFontSize(18);
    doc.setTextColor(40);
    doc.text('DAFTAR LOGISTIK RUMAH SAKIT', 14, 15);
    ...
    // Get table data
    const headers = [
        'No', 'Unit', 'Kategori', 'Jenis Alat/Logistik', 'Merk', 
        'Kode Barang', 'Tanggal Kalibrasi', 'Kadaluarsa Kalibrasi', 
        'Stok', 'Terpakai', 'Satuan', 'Status'
    ];
    ...
    rows.forEach((row, rowIndex) => {
        ...
        for (let i = 0; i < cols.length - 1; i++) {
            // For status column, get the text from the badge
            if (i === 11) {
                const badge = cols[i].querySelector('.status-badge');
                rowData.push(badge ? badge.textContent : '');
            } else {
    ...
    // Save the PDF
    doc.save(`Daftar_Logistik_RS_${new Date().toISOString().slice(0,10)}.pdf`);
}
...
function exportToExcel() {
    ...
    const wscols = [
        ...
        { wch: 15 }, // Unit
        { wch: 15 }, // Kategori
        { wch: 25 }, // Jenis Alat/Logistik
        { wch: 15 }, // Merk
        { wch: 15 }, // Kode Barang
        { wch: 15 }, // Tanggal Kalibrasi
        { wch: 15 }, // Kadaluarsa Kalibrasi
        { wch: 8 },  // Stok
        { wch: 8 },  // Terpakai
        { wch: 8 },  // Satuan
        { wch: 10 }  // Status
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Daftar Logistik");
    
    // Generate Excel file and download
    XLSX.writeFile(wb, `Daftar_Logistik_RS_${new Date().toISOString().slice(0,10)}.xlsx`);
}
            </script>

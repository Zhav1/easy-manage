...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/flowbite@3.1.2/dist/flowbite.min.js"></script> {{-- Keep one Flowbite --}}
...
<body class="min-h-full bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 text-gray-800">
    {{-- CRITICAL FIX: Place these at the very top of body, before any other scripts that might use them --}}
    <script>
        window.authToken = "{{ session('token') }}";
        window.currentUser = {
            id: {{ Auth::user()->id }},
            department_id: {{ Auth::user()->department_id ?? 'null' }}, {{-- Added null coalescing for safety --}}
            hospital_id: {{ Auth::user()->hospital_id ?? 'null' }} {{-- Added null coalescing for safety --}}
        };
    </script>
    <script src="{{ asset('js/tna.js') }}"></script>

    <button class="mobile-menu-btn p-2 rounded-md bg-white shadow-md text-gray-600" onclick="window.toggleMobileMenu()"> {{-- Changed to window.toggleMobileMenu --}}
    ...
    <div class="p-4 md:p-0 mt-8">
        <main class="md:pl-60 pr-5 flex-1 px-4 md:px-6 py-4 md:py-8 mt-0 md:mt-8">
            ...
            <div class="flex flex-wrap gap-2 md:gap-4 mb-6 md:mb-8">
                <button onclick="openAddTnaModal()" id="openAddTnaModalBtn" class="btn-green-tna btn-mobile"> {{-- Added missing ID here --}}
                    <i class="fas fa-plus mr-2"></i>Tambah Data TNA
                </button>
                <button onclick="openAddStaffModal()" id="openAddStaffModalBtn" class="btn-blue-tna btn-mobile"> {{-- Added missing ID here --}}
    ...
    <div id="tnaModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg p-5 w-full max-w-md">
            <div class="flex justify-between items-center mb-3">
                ...
                <button type="button" onclick="closeTnaModal()" class="text-gray-500 hover:text-gray-700 text-lg">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <form id="tnaForm">
                ...
                <div class="mb-3 md:mb-4">
                    ...
                    <textarea id="seminarWorkshopWebinar" rows="2" class="w-full px-3 py-2 md:px-4 md:py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 text-sm"></textarea>
                </div>
                <div class="mb-3 md:mb-4">
                    <label for="pelatihan" class="block text-gray-700 text-xs md:text-sm font-medium mb-1 md:mb-2">Pelatihan</label>
                    <textarea id="pelatihan" rows="2" class="w-full px-3 py-2 md:px-4 md:py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 text-sm"></textarea>
                </div>
                <div class="mb-4 md:mb-6">
                    ...
                    <textarea id="pendidikanLanjutan" rows="2" class="w-full px-3 py-2 md:px-4 md:py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 text-sm"></textarea>
                </div>
                <div class="flex justify-end space-x-2">

...
<body>
    ...
    <h2 class="section-title">Detail Log Harian</h2>
    <table>
        <thead>
            <tr>
                <th>Tipe</th>
        ...
        <tbody>
            @forelse($dailyLogs as $log)
                <tr>
                    ...
                    <td>{{ $log['briefing_conducted'] ?: '-' }}</td>
                    <td>{{ $log['meeting_held'] ?: '-' }}</td>
                    <td>{{ $log['supervision_conducted'] ?: '-' }}</td>
                    <td>{{ $log['handover_done'] ?: '-' }}</td>
                    <td>{{ $log['external_task'] ?: '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="11" style="text-align: center;">Tidak ada data log harian.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
